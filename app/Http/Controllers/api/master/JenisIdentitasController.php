 ...
 *
 * Created on Fri Dec 22 2023 - 21:00:09
...
class JenisIdentitasController extends Controller
{
    function data(Request $request)
    {
        try {
            ...
            $nLimit = 10;
            $vaData = DB::table('jenisidentitas')
                ->select(
                    'Kode',
                    'Keterangan'
                );
            ...
            if ($vaData) {
                ...
                Func::writeLog('Jenis Indentitas', 'data', $vaRequestData, $vaRetVal, $cUser);
    ...
    function store(Request $request)
    {
        try {
            ...
            if ($nReqCount > 2 || $nReqCount < 2) {
                ...
                Func::writeLog('Jenis Indentitas', 'store', $vaRequestData, $vaRetVal, $cUser);
                // return $vaRetval;
                return response()->json(['status' => 'error']);
            }
            ...
            if ($vaValidator->fails()) {
                ...
                Func::writeLog('Jenis Indentitas', 'update', $vaRequestData, $vaRetVal, $cUser);
                // return response()->json($vaRetVal);
                return response()->json([
                    'status' => 'error',
                    'message' => $vaErrorMsgs
    ...
    function update(Request $request)
    {
        try {
            ...
            if ($vaData) {
                ...
                JenisIdentitas::where('Kode', '=', $cKode)->update($vaArray);
                // JIKA REQUEST SUKSES
                $vaRetVal = [
                    "status" => "00",
                    "message" => "SUKSES"
    ...
    function delete(Request $request)
    {
        try {
            ...
            if ($nReqCount > 1 || $nReqCount < 1 || $vaRequestData['Kode'] == null || empty($vaRequestData['Kode'])) {
                ...
                Func::writeLog('Jenis Indentitas', 'delete', $vaRequestData, $vaRetVal, $cUser);
                // return $vaRetVal;
                return response()->json(['status' => 'error']);
            }
            $vaData = JenisIdentitas::findOrFail($vaRequestData['Kode']);
            $vaData->delete();
            $vaRetVal = [
                "status" => "00",
                "message" => "SUKSES"
