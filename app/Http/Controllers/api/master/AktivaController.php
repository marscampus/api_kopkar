 ...
 *
 * Created on Tue Mar 26 2024 - 03:03:54
...
class AktivaController extends Controller
{
	public function data(Request $request)
	{
		...
		try {
			...
			if ($vaData) {
				...
				Func::writeLog('Master Aktiva', 'data', $vaRequestData, $vaRetVal, $cUser);
	...
	{
		$Aktiva = Aktiva::where('Kode', $Kode)->update([
	...
	{
		try {
			$Aktiva = Aktiva::findOrFail($request->Kode);
			$Aktiva->delete();
			return response()->json(['status' => 'success']);
		} catch (\Throwable $th) {
			return response()->json(['status' => 'error']);
	...
	public function getDataEdit(Request $request)
	{
		$Kode = $request->Kode;
		try {
			$barcode = Aktiva::where('Kode', '=', $Kode)->first();
			if ($barcode) {
